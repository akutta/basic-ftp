...
class Client {
     ...
     */
    async downloadDir(localDirPath, regex, skipPreviouslyDownloaded) {
        if ( !regex ) {
          regex = /.*/
        }
        ...
        for (const file of await this.list()) {
            ...
            if (file.isDirectory) {
                await this.cd(file.name);
                await this.downloadDir(localPath, regex, skipPreviouslyDownloaded);
                await this.send("CDUP");
            }
            else {
              if ( localPath.match(regex) ) {
                if ( !skipPreviouslyDownloaded || !fs.existsSync(localPath) ) {
